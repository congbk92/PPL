...
dowhileStmt: DO listStmt WHILE expr SM ;
listStmt: stmt tailListStmt ;
tailListStmt: stmt tailListStmt | ;
forStmt: FOR LB expr SM expr SM expr RB stmt ;
breakStmt: BREAK SM ;
continueStmt: CONTINUE SM ;
returnStmt: RETURN expr? SM ;
exprStmt: expr SM ;

expr: lhs ASSIGN expr | binaryExpr;
...
indexExpr: (ID | invocExpr) LS expr RS | higherExpr;
higherExpr: LB expr RB | INTLIT | FLOATLIT| BOOLEANLIT | STRINGLIT | ID | invocExpr;
